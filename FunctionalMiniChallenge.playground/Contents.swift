...
let freshmen = students.filter({$0.year == StudentYear.freshman }).map({return $0.firstName + " " + $0.lastName})
print(freshmen)
...
let fullNames = students.map({return $0.firstName + " " + $0.lastName})
print(fullNames)

// TODO: - Return the average grade from all the students using the reduce function divided by the count

let averageGrade = students.map({return $0.grade}).reduce(0, +) / students.count
print(averageGrade)
